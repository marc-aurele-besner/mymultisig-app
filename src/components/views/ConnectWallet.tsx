...
import { Text, SimpleGrid, Box, VStack, Image } from '@chakra-ui/react'
          ...
          {connectors.map((connector, index) => {
            ...
            return (
              <MotionBox
                ...
                bg='whiteAlpha.50'
                border='1px solid'
                borderColor={isPending ? 'brand.400' : 'whiteAlpha.100'}
                cursor={isDisabled ? 'not-allowed' : 'pointer'}
                opacity={isDisabled ? 0.5 : 1}
                sx={{ transition: 'all 0.2s ease' }}
                _hover={{
                  bg: isDisabled ? 'whiteAlpha.50' : 'whiteAlpha.100',
                  borderColor: isDisabled ? 'whiteAlpha.100' : 'brand.400',
